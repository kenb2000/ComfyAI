[CmdletBinding()]
param(
    [switch]$AcquireOptionalModels,
    [switch]$SkipComfyUI,
    [switch]$SkipVenv,
    [switch]$NoConfigurePorts,
...
if ($AcquireOptionalModels) { $ScriptArgs += "--acquire-optional-models" }
if ($SkipComfyUI) { $ScriptArgs += "--skip-comfyui" }
if ($SkipVenv) { $ScriptArgs += "--skip-venv" }
if ($NoConfigurePorts) { $ScriptArgs += "--no-configure-ports" }
...
Write-Host "Running ComfyUIhybrid bootstrap from $RepoRoot"
